     ...
     <!-- 分类end -->
			...
			<div class="head_hot_search hidden_xs">
				<ul class="pops_list">
					...
					<?php $_5e2ac7fd54173=explode(',',$maccms['search_hot']); if(is_array($_5e2ac7fd54173) || $_5e2ac7fd54173 instanceof \think\Collection || $_5e2ac7fd54173 instanceof \think\Paginator): if( count($_5e2ac7fd54173)==0 ) : echo "" ;else: foreach($_5e2ac7fd54173 as $key=>$vo): if($key < 5): ?>
        ...
        <!-- 筛选 -->
			<div class="art_navlist">
		    	<ul class="clearfix">
		    		...
		    		<li <?php if(($vo['type_id'] == $GLOBALS['type_id'])): ?>class="active"<?php endif; ?>><a href="<?php echo mac_url_type($vo); ?>"><?php echo $vo['type_name']; ?>首页</a></li>
		    		<?php endforeach; endif; else: echo "" ;endif; $__TAG__ = '{"parent":"'.$vo['type_id'].'","order":"asc","by":"sort","id":"vo","key":"key"}';$__LIST__ = model("Type")->listCacheData($__TAG__); if(is_array($__LIST__['list']) || $__LIST__['list'] instanceof \think\Collection || $__LIST__['list'] instanceof \think\Paginator): $key = 0; $__LIST__ = $__LIST__['list'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($key % 2 );++$key;?>						 
					<li <?php if(($vo['type_id'] == $GLOBALS['type_id'] || $vo['type_id'] == $GLOBALS['type_pid'])): ?>class="active"<?php endif; ?>><a href="<?php echo mac_url_type($vo); ?>" class="text_muted"><?php echo $vo['type_name']; ?></a></li>
					<?php endforeach; endif; else: echo "" ;endif; ?>
		    	</ul>
		    	<?php if($conch['theme']['weixin']['btn'] == 1): ?>
		    	<div class="wx_ewm hidden_xs">
                	<p>关注我们</p>
                	<p><img class="wx_img" src="<?php echo mac_url_img($conch['theme']['weixin']['qrcode']); ?>"></p>
                </div>
                <?php endif; ?>
		    </div>
		<!-- end 筛选 -->
		...
		<div class="art_list_row">
             <div class="pannel article_box">
                  <header class="article_header">
                      <h1 class="article_title"><?php echo $obj['art_name']; ?></h1>
                      <div class="article_meta">
                           <span class="item"><?php echo mac_default($obj['art_from'],'网络'); ?></span>
                           <span class="item"><?php echo date('Y-m-d H:i',$obj['art_time']); ?></span>
                           <span class="item hidden_xs"><?php echo $obj['type']['type_name']; ?></span>
                           <span class="item hidden_xs">阅读 <?php echo $obj['art_hits']; ?></span>
                      </div>
                  </header>
                  <?php if($conch['theme']['ads']['btn']==1||$conch['theme']['ads']['btn']==2&&$user['group_id'] < 3): if($conch['theme']['ads']['art_w']['btn'] == 1): ?>
	<div class="ads_art clearfix" style="margin: 0.2rem 0;">
		<div id="1001" class="ads ads_w">
			<?php echo $conch['theme']['ads']['art_w']['content']; ?>
		</div>
	</div>
	<script type="text/javascript">var o = document.getElementById("1001");var h = o.offsetHeight; if (h > 0) {} else {$(".ads_art").addClass("hide");}</script>
<?php endif; endif; ?>
                  <article class="article_content"> 
					  <div><?php echo mac_url_content_img($obj['art_page_list'][$param['page']]['content']); ?></div> 
                  </article>
                  <div class="art_zan_box">
                  <?php if($obj['type']['type_id'] != 17): ?>
                  <div class="digg_zan">
                      <a class="digg_link" data-id="<?php echo $obj['vod_id']; ?><?php echo $obj['art_id']; ?><?php echo $obj['topic_id']; ?>" data-mid="<?php echo $maccms['mid']; ?>" data-type="up" href="javascript:;">
						  赞<em class="digg_num"><?php echo $obj['vod_up']; ?><?php echo $obj['art_up']; ?><?php echo $obj['topic_up']; ?></em>
                      </a>
                  </div>
...
<?php endif; if($obj['type']['type_id'] != 17): ?>
	...
	<?php if($conch['theme']['share']['bdbtn'] == 1): if($maccms['mid'] == 2): ?>
		...
		<?php endif; endif; ?>
              ...
              <div class="pannel art_pren_list clearfix">
				  <?php $Prev = $obj['art_id']+1; $next = $obj['art_id']-1; $__TAG__ = '{"num":"1","ids":"'.$Prev.'","id":"vo","key":"key"}';$__LIST__ = model("Art")->listCacheData($__TAG__); if(is_array($__LIST__['list']) || $__LIST__['list'] instanceof \think\Collection || $__LIST__['list'] instanceof \think\Paginator): $key = 0; $__LIST__ = $__LIST__['list'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($key % 2 );++$key;?><a class="art_prev" href="<?php echo mac_url_art_detail($vo); ?>"><span>上一篇</span><div class="art_name_prev"><?php echo $vo['art_name']; ?></div></a><?php endforeach; endif; else: echo "" ;endif; $__TAG__ = '{"num":"1","ids":"'.$next.'","id":"vo","key":"key"}';$__LIST__ = model("Art")->listCacheData($__TAG__); if(is_array($__LIST__['list']) || $__LIST__['list'] instanceof \think\Collection || $__LIST__['list'] instanceof \think\Paginator): $key = 0; $__LIST__ = $__LIST__['list'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($key % 2 );++$key;?><a class="art_next" href="<?php echo mac_url_art_detail($vo); ?>"><span>下一篇</span><div class="art_name_next"><?php echo $vo['art_name']; ?></div></a><?php endforeach; endif; else: echo "" ;endif; ?>
		      </div>
              <?php if($GLOBALS['config']['comment']['status'] == 1): ?>
              <!-- 评论 -->
              <div class="pannel clearfix" id="pinglun" >
				...
				<div class="mac_comment comm_list comm_list_box" data-id="<?php echo $obj['art_id']; ?>" data-mid="<?php echo $maccms['mid']; ?>" ></div>
				<script>
					$(function(){
		...
		<div class="art_sider_row">
			<?php if($conch['theme']['ads']['btn']==1||$conch['theme']['ads']['btn']==2&&$user['group_id'] < 3): if($conch['theme']['ads']['art_r']['btn'] == 1): ?>
	<div class="ads_rbox hidden_xs hidden_mi">
		<div class="pannel ads_rmk clearfix">
			<div class="ads_rmkc">
				<div id="1002" class="ads right_ads">
					<?php echo $conch['theme']['ads']['art_r']['content']; ?>
				</div>
			</div>
		</div>
    </div>
<?php endif; endif; ?>
			<div class="pannel clearfix">
				...
				<ul class="art_relates clearfix">
				        <?php if($conch['theme']['ads']['btn']==1||$conch['theme']['ads']['btn']==2&&$user['group_id'] < 3): if($conch['theme']['ads']['artlist']['btn'] == 1): ?>
<li>
   <a target="_blank" rel="nofollow" href="<?php echo $conch['theme']['ads']['artlist']['link']; ?>" title="<?php echo $conch['theme']['ads']['artlist']['title']; ?>">
   <div class="artlr_tit"><p class="artlr_b"><?php echo $conch['theme']['ads']['artlist']['title']; ?></p><p class="artlr_name">随便看看</p></div>
   <div class="artlr_pic lazyload" data-original="<?php echo mac_url_img($conch['theme']['ads']['artlist']['pic']); ?>">
   <?php if($conch['theme']['ads']['artlist']['tbtn'] == 1): ?>
       <span class="ad_tips">广告</span>
   <?php endif; ?>
   </div>
   </a>
</li>
<?php endif; endif; $__TAG__ = '{"num":"6","ids":"'.$obj['art_rel_art'].'","order":"desc","by":"hits","id":"vo","key":"key"}';$__LIST__ = model("Art")->listCacheData($__TAG__); if(is_array($__LIST__['list']) || $__LIST__['list'] instanceof \think\Collection || $__LIST__['list'] instanceof \think\Paginator): $key = 0; $__LIST__ = $__LIST__['list'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($key % 2 );++$key;if($vo['art_pic'] == ''): ?>
<li class="no_artpic">
